...
<script>
    $('ul.showProjectTemplateTabs .projectTemplateMilestone').addClass('tab-current');
    var table = $('#timelog-table').dataTable({
        responsive: true,
        processing: true,
        serverSide: true,
        ajax: '{!! route('admin.project-template-milestone.data', $project->id) !!}',
        deferRender: true,
        language: {
    ...
    $('#save-form').click(function () {
        $.easyAjax({
            url: '{{route('admin.project-template-milestone.store')}}',
            container: '#logTime',
            type: "POST",
    ...
    $('body').on('click', '.sa-params', function () {
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                var url = "{{ route('admin.project-template-milestone.destroy',':id') }}";
...
//                                    swal("Deleted!", response.message, "success");
    ...
    $('body').on('click', '.edit-milestone', function () {
        ...
        var url = '{{ route('admin.project-template-milestone.edit', ':id')}}';
        url = url.replace(':id', id);
    ...
    $('body').on('click', '.milestone-detail', function () {
        var id = $(this).data('milestone-id');
        var url = '{{ route('admin.project-template-milestone.detail', ":id")}}';
        url = url.replace(':id', id);
